...
- [ ] Stage 4.5 — Look & Feel Foundation
    ...
    - [ ] Stage 4.5e — Person screen — built and tested; known gaps before check-off, all to fix first in the next chat: (a) back-navigation takes too many taps to get from deep inside a family back to Home, needs a nested-navigator fix; (b) FamilyDetailScreen's form keyboard handling needs improving (screen should push up/down, not cover fields or dismiss on scroll); (c) "Add a Person" should become a floating + button per the mockup rules, instead of a permanent inline form — bundle with fix (a) since both touch navigation/screen structure; (d) the "link to an existing person" UI has confusing wording ("tap here to hide" / "Show people already added") and an awkwardly-placed Cancel button that looks like it cancels linking rather than the whole form — needs clearer copy and a clearer Cancel location; also needs validation to stop contradictory links (e.g. linking two people as spouses when they're already linked as parent/child), which caused a duplicate-key crash on the Person screen (worked around defensively in PersonScreen.js by deduping the relatives list — real fix is the validation itself).
